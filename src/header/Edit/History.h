...
//
//왜 씬 전체를 스냅샷으로 뜨지 않는가:
//  가장 쉬운 구현은 편집할 때마다 씬을 통째로 복사해 두는 것이다. 오브젝트만 있으면 그래도 된다.
//  걸리는 건 정점 편집이다. 정점 10만 개짜리 메시를 한 번 끌 때마다 2.4MB가 쌓이면
//  스택 100칸에 240MB가 된다. 그래서 "무엇이 바뀌었는지"만 기록한다 —
//  오브젝트는 스냅샷(작다), 정점은 바뀐 것만 델타로.
//
//액션 하나 = 사용자가 "한 번 했다"고 느끼는 단위.
//마우스를 누른 채 정점을 30프레임 끌어도 Ctrl+Z 한 번에 원위치여야 한다.
//그래서 정점 편집은 EditMode::BeginStroke / CommitStroke로 한 덩어리로 묶는다.

//오브젝트 하나의 상태 + 목록에서의 자리.
//자리까지 기억하는 이유: 안 그러면 되살린 오브젝트가 아웃라이너 맨 밑으로 가서 순서가 뒤섞인다.
